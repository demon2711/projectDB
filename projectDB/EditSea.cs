...
namespace projectDB
{
    public partial class EditSea : Form
    {
        public EditSea()
        {
            InitializeComponent();
        ...
        private void EditSea_Load(object sender, EventArgs e)
        {
            var con = new SQLiteConnection(cs);
            con.Open();
            int count = Convert.ToInt32(AddAndEdit.NowNumber) + 1;
            string query = "SELECT * FROM ObjectSourceSea WHERE id=" + count;
            ...
            nickNameSea.Text = values[45];
            lengthShip.Text = values[46];
            heightShip.Text = values[47];
            displacement.Text = values[48];
            codeMMSI.Text = values[49];
            description.Text = values[50];
            informationSea.Text = values[51];
            pathRLC.Text = values[52];
        }
        ...
        private void Ok_Click(object sender, EventArgs e)
        {
            ...
            cmd.CommandText = "SELECT * FROM ObjectSourceSea WHERE id=" + id;
            SQLiteDataReader dr;
            dr = cmd.ExecuteReader();
            string path = "";
            while (dr.Read())
            ...
            cmd.CommandText = "UPDATE ObjectSourceSea Set Object=@ObjectDB, Source=@SourceDB, FreqFrom=@FreqFrom," +
                ...
                "LengthShip=@LengthShip, HeightShip=@HeightShip, Displacement=@Displacement, MMSI=@MMSI, " +
                "Description=@Description, Information=@Information, Path=@Path WHERE id = @id";
            cmd.Prepare();
            ...
            string HeightShip = heightShip.Text;
