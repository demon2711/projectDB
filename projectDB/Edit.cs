...
namespace projectDB
{
    public partial class Edit : Form
    {
        ...
        private void Edit_Load(object sender, EventArgs e)
        {
            ...
            string query = "SELECT * FROM ObjectSource WHERE id=" + count;
            ...
            nickName.Text = values[45];
            codeZA.Text = values[46];
            codeIKAO.Text = values[47];
            speed.Text = values[48];
            RLO.Text = values[49];
            minHeightLimit.Text = values[50];
            maxHeightLimit.Text = values[51];
            mainArmament.Text = values[52];
            purpose.Text = values[53];
            otherParam.Text = values[54];
            pathRLC.Text = values[55];
        ...
        private void Ok_Click(object sender, EventArgs e)
        {
            ...
            cmd.CommandText = "SELECT * FROM ObjectSource WHERE id=" + id;
            SQLiteDataReader dr;
            dr = cmd.ExecuteReader();
            string path = "";
            while (dr.Read())
            ...
            cmd.CommandText = "UPDATE ObjectSource Set Object=@ObjectDB, Source=@SourceDB, FreqFrom=@FreqFrom," +
                ...
                "CodeZA=@CodeZA, CodeIKAO=@CodeIKAO, Speed=@Speed, RLO=@RLO, " +
                "MinHeightLimit=@MinHeightLimit, MaxHeightLimit=@MaxHeightLimit, " +
                "MainArmament=@MainArmament, Purpose=@Purpose, OtherParam=@OtherParam, Path=@Path WHERE id = @id";
            cmd.Prepare();
            ...
            string RLOEdit = RLO.Text;
            string MinHeightLimit = minHeightLimit.Text;
            string MaxHeightLimit = maxHeightLimit.Text;
            string MainArmament = mainArmament.Text;
            ...
            cmd.Parameters.AddWithValue("RLO", RLOEdit);
            cmd.Parameters.AddWithValue("@MinHeightLimit", MinHeightLimit);
            cmd.Parameters.AddWithValue("@MaxHeightLimit", MaxHeightLimit);
            cmd.Parameters.AddWithValue("@MainArmament", MainArmament);
